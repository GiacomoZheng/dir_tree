...
impl MetaData {
    pub fn parse(s: String) -> MetaData {
        YFM::parse(s.as_str()).unwrap().metadata
    }

    pub fn with_tag(&self, tag: &String) -> bool {
        self.tags.contains(tag)
    }

    pub fn try_add_to_set(self, set: &mut HashSet<MetaData>) -> Result<(), &'static str> {
        if !set.insert(self) {
...
#[test] fn t_actindex_parse() {
    ...
    tags:
      ...
      - 一般代数几何
    ---";
    let md = MetaData::parse(s.to_string());
